...
#### 7.9.1 Proc-casting engine (GitHub #10) — the headline branding mechanic

The §7.9 principle — *"Brands change proc frequency / behavior / triggers, never flat ±% damage as a
primary lever"* — requires that a brand's expression is a **spell it casts on a combat opportunity**,
not a passive multiplier. Effect application (§03) initially shipped only placeholder flat ±% damage
multipliers (`EffectScripts.cpp`, `MasteryCombatScripts.cpp`); the proc engine is the real mechanic:

```
combat opportunity (this slice: a MELEE swing dealt by a Player)
   → PPM roll        (ProcChancePerSwing(ppm, weaponSpeedS), built on §14.3 ExpectedProcs)
   → window gate     (IsWindowActive — §7.9 "no passive uptime")
   → expressibility  (EffectStrength > 0 — anti-P2W CanExpressBrand gate, §7.5)
   → cast payload    (a reused 3.3.5a spell shell + a base value scaled by effect strength)
```

**Pure model (`core/branding/proc/ProcEngine.h`).** The decision is standalone-testable with an
**injected roll** (no `std::rand`, no AC types):

```cpp
double ProcChancePerSwing(double ppm, double weaponSpeedS);          // reuses ExpectedProcs
struct ProcOpportunity { double ppm, weaponSpeedS, effectStrength; bool windowActive;
                         uint32_t spellId; int32_t baseValue; };
...
`baseValue * effectStrength` (fed to `SPELLVALUE_BASE_POINT0`), so a level-0 / non-expressible
character casts nothing — the anti-P2W and §7.9 "no passive uptime" invariants hold in the pure layer.

**Adapter (`ProcEngineScripts.cpp` + `ProcEngineMgr`).** A `UnitScript::ModifyMeleeDamage` hook: when
the attacker is a `Player`, it reuses the **same knowledge/expressibility gate as `EffectMgr`**
(`ResolveActiveProfile`), fills the opportunity from the live state it can read (config PPM, the
...
- **Custom `item_template` entries — reserved band `190000–190999`** (decided; widened from `190199`
  for the archetype-set sub-bands, #1 — 8 armour-class/role sets × 9 slots overflow the original
  40-slot output sub-band). All mod-branding custom items draw from this band so entries can't collide
  with other modules or upstream content:
  - generic resources `190000–190001` (Material, Fragment);
  - starter Branded outputs `190010+`, starter recipe patterns `190050+` (the shipped chest trio);
  - **per-school Fragments `190100–190114`** (one per `BrandId`, laid out as
    `SchoolFragmentBaseItemId + BrandId`; sub-band reserved through `190149`);
  - **archetype set outputs `190200–190499`** and **set recipe patterns `190500–190799`** (#1):
    armour pieces grouped 10-per-set (`base + set*10 + slot`) at `190200`/`190500`, plus one
    Blacksmithing-crafted main-hand **weapon** per set at `190280`/`190580` (`base + set`);
  - `190800–190999` reserved for future content.

  Map every config-wired entry to a `Branding.<Sub>.*ItemId` config key (§16.3); never hard-code an
  entry in C++. The per-school-Fragment base is shared by the C++ adapter
  (`Branding.Economy.SchoolFragmentBaseItemId`) and the generator (`tools/branding-craft`
  `SCHOOL_FRAGMENT_BASE`) and **must** stay equal. The set sub-bands, like the starter trio, are
  generated and band-checked by `tools/branding-craft` (`SET_OUTPUT_BAND` / `SET_PATTERN_BAND`).
- **Custom craft spells — reserved band `1900000–1900099`** (decided, #29). Native profession craft
  ...
  **`spell_dbc`** and **`skilllineability_dbc`** (`DBCStores.cpp` `LoadFromDB`) — *not* a
  ...
  single catalog (`tools/branding-craft`) so reagents cannot drift. **Band pressure:** the starter trio
  uses `1900010–1900012`, the archetype set armour `1900013–1900084`, and the set weapons
  `1900085–1900092` — so the 100-slot craft-spell band is ~83% full (83/100). It must **not** be
  widened past `1900099` without re-verifying collision-freedom against the client DBC — the `1900100+`
  range is the SLA band (see the `branding-craft` README's client-verification note).
- **Module content SQL home.** Module-owned content (resources, Branded outputs, recipe patterns,
  ...
  *not* the core `data/sql/updates/pending_db_*` tree — the latter is core content and its SQL linter
  forbids `DELETE FROM item_template`. The custom `branding_recipe` table is created + seeded in the
  **same module db-world tree** (`2026_06_20_01_branding_recipe.sql` creates it, `..._06_22_03` adds
  the `school` column, `2026_07_14_00_branding_recipe.sql` seeds the full catalog incl. the #1 sets +
  weapons). Its rows are generated by `branding-craft recipe-sql` from the same catalog as the craft
  spells, so the mirror can't drift (#29 lockstep). *(Historical note: earlier docs placed this table
  in a #09 core pending rev; the actual, shipped home is module db-world.)*
